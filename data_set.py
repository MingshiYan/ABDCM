...
class TrainDate(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        total = []
        user = idx + 1
        inter = self.behavior_dict['all'].get(str(idx + 1), None)
        pos_list = self.behavior_dict[self.behaviors[-1]].get(str(idx + 1), None)
        if pos_list is None:
            total.append([0, 0, 0])
        else:
            pos = random.sample(pos_list, 1)[0]
            neg = random.randint(1, self.item_count)
            while np.isin(neg, inter):
                neg = random.randint(1, self.item_count)
            total.append([user, pos, neg])
        for i in range(len(self.behaviors) - 1):
            items = self.behavior_dict[self.behaviors[i]].get(str(user), None)
            if items is None:
                signal = [0, 0, 0]
            else:
...
class DataSet(object):
    ...
    def train_dataset(self):
        ...
        return TrainDate(self.user_count, self.item_count, self.train_behavior_dict, self.behaviors)
